...
AddEventHandler('onClientResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		exports["pulsar-kbs"]:Add("voip_cycleproximity", "Z", "keyboard", "Voice - Cycle Proximity", function()
			if _characterLoaded and PLAYER_CONNECTED then
				exports["pulsar-voip"]:Cycle()
			end
		end)

		exports["pulsar-kbs"]:Add("voip_radio", "CAPITAL", "keyboard", "Voice - Radio - Push to Talk", function()
...
AddEventHandler("Characters:Client:Spawn", function()
	...
	exports['pulsar-hud']:UpdateVoip(2, false)
...
AddEventHandler("Characters:Client:Logout", function()
	...
	exports['pulsar-core']:LoggerInfo("VOIP", "Disconnecting From Mumble (Character Logging Out)")
end)
...
function UpdateVOIPIndicatorStatus()
	...
	exports['pulsar-hud']:UpdateVoip(stage, talking, indicatorIcon)
end
...
exports("MicClicks", function(on, isLocal)
	if on then
		exports["pulsar-sounds"]:PlayOne("mic_click_on.ogg", 0.1 * (VOIP_SETTINGS?.RadioClickVolume or 1.0))
	else
		exports["pulsar-sounds"]:PlayOne("mic_click_off.ogg", 0.1 * (VOIP_SETTINGS?.RadioClickVolume or 1.0))
	end
end)
