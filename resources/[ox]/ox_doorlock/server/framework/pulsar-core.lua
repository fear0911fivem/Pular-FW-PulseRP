...
function GetCharacterId(player)
    ...
    local char = exports['pulsar-characters']:FetchCharacterSource(player.source)
    if not char then
        return -1
    end
...
function IsPlayerInGroup(player, filter)
    ...
    if type(filter) == "string" then
        return exports['pulsar-jobs']:HasJob(player.source, filter) ~= false
    end
    ...
    for job, requiredGrade in pairs(filter) do
        if job ~= "workplace" and job ~= "permissions" and job ~= "onduty" then
            local jobData = exports['pulsar-jobs']:HasJob(
                player.source,
                job,
            ...
            local dutyData = exports['pulsar-jobs']:DutyGet(player.source, job)
            local isDutyValid = (not filter.onduty) or (dutyData ~= false)

            local hasPermission = true
            if filter.permissions then
                hasPermission = exports['pulsar-jobs']:HasPermissionInJob(
                    player.source,
                    job,
                    filter.permissions
                )
